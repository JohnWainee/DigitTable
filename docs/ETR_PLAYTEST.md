...
- **Status:** Fable deliverable for issue #14 task F04. Written against `docs/ETR_RULES_MATRIX.md` (F01) and `docs/ETR_SESSION_FLOW.md` (F02); reconciled with A02 (`sonnet-a/a02` @ af20261) for request/ownership/state names. Template command names, new error codes, and fixture ids still follow F01/F02 and are re-checked when B02/B03 publish (marked ⚠ where a name may change). Rules expectations are page-cited to the matrix rows (e.g. "F01 A7") and do not depend on the implementation.
...
2. **Disconnect after Confirm, before the response.** Sam taps Confirm; the request reaches the server; the response is lost. On reconnect the outbox entry is `disconnected` (A02 `SessionRequestState`) → receipt query → `accepted` → the resolved summary renders; the client never sends a second `AllocateResults` with a new commandId. Authority shows one `ActionResolved` for that roll.
