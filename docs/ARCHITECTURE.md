...
`AUTH_REQUIRED`, `ROLE_FORBIDDEN`, `REVISION_CONFLICT`, `ROLL_ALREADY_RESOLVED`, `TEMPLATE_VERSION_MISMATCH`, `ROOM_ARCHIVED`, `RATE_LIMITED`, `PAYLOAD_TOO_LARGE`, `UNKNOWN_ACTION`, `INVALID_ALLOCATION`, and the admission family added in Phase 2 PR 3 (`ROOM_FULL`, `ADMISSION_CLOSED`, `ROOM_NOT_FOUND`, `INVALID_PASSPHRASE`, `GM_SEAT_TAKEN`, `ROOM_DATA_INVALID` for a persisted document that fails runtime validation, and `INVALID_REQUEST` for an untrusted payload that fails validation before any read) map to actionable client states — kept in sync with `packages/contracts/src/errors.ts`'s `STABLE_ERROR_CODES`, the canonical list. Never expose stack traces or hidden payload details.
...
admissionThrottle/code-{h}/byIp/{h}  fixed-window join-attempt counters (per code+IP, per IP,
admissionThrottle/ip-{h}/scope/all     per verified UID); document IDs are SHA-256 hashes,
admissionThrottle/uid-{h}/scope/all    never the submitted value; service-only
...
Every persisted document the admission authority reads — `roomCodes/{code}`, `authority/current`, `uidBindings/{uid}`, the secret hashes, and the throttle counters — is runtime-validated and fails closed (Phase 2 PR 3 review, finding 3): a document that exists but is malformed (an unrecognized `roomStatus`/`admissionStatus`, a non-integer or negative `participantCount`, a non-boolean `tableSeatClaimed`, an absent or empty `gmMemberId` key — absence must never read as an open GM seat — an empty `memberId`, an `iterations` outside 1–1,000,000) denies the request with `ROOM_DATA_INVALID` and writes nothing; so does a room whose client-readable `meta/current` mirror is missing, since a GM claim updates it and must never create a partial one. Malformed data never reads as `active`, `open`, zero occupancy, "no binding", "verified", or a fresh throttle window. Only a genuinely absent document carries the narrower meaning of "not found", "no binding", or "no secret to verify against".

The `admissionThrottle` tree holds the join throttle (section 11): three fixed 60-second-window counters consumed together in one transaction before the admission transaction, so a throttled caller never reads `authority/current` or a secret hash — at most 20 attempts per (submitted room code, caller IP), which bounds passphrase brute force against one code; 100 per caller IP across all codes, which bounds room-code enumeration from one address; and 60 per verified anonymous UID, an unspoofable bound that holds even where the resolved client IP cannot be trusted (Phase 2 PR 3 second pass, T1/C1/C6). A request denied by any bucket consumes nothing. Document IDs are SHA-256 hashes of the submitted code, address, and UID, so no untrusted value is stored as a path segment or can exceed Firestore's ID limit; each document carries an `expiresAt` timestamp for a Firestore TTL policy (enable it on the `byIp` and `scope` collection groups before public preview). Keyed on the submitted code rather than a resolved room ID, and kept outside `rooms/`, so guesses at codes that resolve to no room are bounded too. Callers whose IP cannot be resolved share one `unknown` bucket rather than being exempt; the UID bucket still bounds each of them individually. Room codes are validated to letters, digits, and hyphens before any of this runs, and display names to visible, non-control text.
...
3. The `admitMember`/`claimSeat` callables in `apps/functions` require a signed-in identity, validate the payload, apply the throttle (per code+IP, per IP, per verified UID), then in one transaction resolve the service-only code index, verify only the secret the requested capability needs (room passphrase for players and the GM claim, the separate table code for the table seat), and validate room status, capacity, and admission policy. The seat's recovery credential is minted and hashed before the transaction so no slow hash runs while `authority/current` is locked. App Check is monitored, not yet enforced (section 11).
