...
- **Last updated:** 2026-09-24 by Claude (Sonnet 5, worktree `digitable-sonnet-u-reskin-followup-20260924`) after a third independent mobile-pop-out re-audit found no new defect and no source change was needed (see "Second mobile pop-out follow-up audit, 2026-09-24" below) — this entry also recommends pausing the recurring audit schedule. Previously: 2026-09-24 by Claude (Sonnet 5) after an independent mobile-pop-out reskin audit found the deployed UI sound and fixed a stale audit-tooling reference that had been silently skipping the modal/pop-out check (see "Mobile pop-out independent audit, 2026-09-24" below). Before that: 2026-09-19 by Codex after the sourcebook-roster implementation and independent review.
...
## Mobile pop-out independent audit, 2026-09-24 (this worktree, `worktree-sonnet-n-reskin-hourly-20260924`)

**Scope:** an independent audit of the deployed ink-black reskin's mobile pop-out UI/UX (the class the 2026-09-18 `sonnet-d` review targeted: selects, menus, disclosures, modals/drawers, option lists, allocation/action pickers), per `AGENTS.md`'s review-before-changing and scope-discipline rules. No engine, contracts, template, Functions, or rules file was touched; the only source change is dev tooling (below).

1. **Inventory (unchanged from the 2026-09-18 review, re-verified by reading the current source):** the player flow has no native `<select>`, dropdown, or menu — only checkbox lists and the custom `AllocationStepper` spinbutton, plus one `<details>` "Why?" disclosure. The GM console (`GmToolsPanel.tsx`, `SceneDirector.tsx`) has the app's only native `<select>` elements (all correctly `<label htmlFor>`-paired, browser-owned popups, ellipsised option text per `styles.css`'s documented rationale). The app's one modal/pop-out is the GM correction sheet (`CorrectionDialog` on `SheetDialog.tsx`), the one `role="dialog"` in the app, already hardened through three review rounds (commits `c215367`, `1d109a0`, `c632597`, `7b58968`). No new pop-out pattern exists.
2. **Found and fixed a real gap in the audit tooling itself, not the app:** `scripts/playtest/ui-audit.mjs`'s `openCorrection()` located the roster row to open the correction sheet from by matching character name `/^rook/i` — the Phase 1A placeholder character. The sourcebook roster (`5e8907b`) replaced "Rook" with six named characters, so that lookup has timed out on every run since, silently skipping the entire modal/pop-out audit (the script's single deepest mobile check: safe-area, pinch-zoom, on-screen-keyboard, 200%-text, focus trap) while the rest of the sweep still reported a pass. Confirmed live: pre-fix, a full run against `https://digitable.signal-bleed.com` failed with `timed out waiting for: Rook's Correct button` after 108 states with the modal case never reached. Fixed to open the first roster row instead (any character produces an equivalent dialog for this audit's geometry/focus/safe-area checks — verified by reading `RosterPanel.tsx` and `CorrectionDialog.tsx`, and independently re-verified by a fresh reviewer agent: **approve, no blocking finding**, one non-blocking nit about the comment's precision if a future character ever shipped with zero items and zero injuries).
3. **Fresh live evidence after the fix**, `node scripts/playtest/ui-audit.mjs --base https://digitable.signal-bleed.com`: **150 states, 1,470 controls audited, 0 control issues, 0 overflow states, 0 hard axe violations, 0 failures.** The modal audit now runs to completion at phone-small (320x568), phone (375x812), tablet, and desktop, including the on-screen-keyboard emulation and focus-return-on-close checks, all passing (`dialogInsideViewport`, `actionsVisibleWithoutScrolling`, `noPageOverflow`, `actionTargets44`, `reasonReachableAfterScroll`, `rootScrollLocked`, `backgroundInert`, `focusInsideDialog` all true at every case). Two pre-existing, already-documented non-gating notes remain, neither new: the 320px+200%-text-zoom combination (`sonnet-d` review R3/N4, a limit of the unchanged console layout, not the sheet) and the `page-has-heading-one` best-practice item on the nonexistent-room route (F7, open for John since 2026-09-18).
4. **Gates:** `npm run check` — format, lint, typecheck, **690 tests passed | 11 todo** (71 files passed, 1 skipped), unchanged from the last recorded baseline (no engine/template/contracts/app source touched). `npm run build` — passed (Functions esbuild 216.6kb, web build 138 modules, existing >500kB chunk warning only). `npm run test:emulator` — **could not get exclusive emulator ports in this run**: multiple other concurrent sessions (`ListAgents` shows five peers, several actively running the near-identical hourly reskin audit) already held ports 8080/9000/9099 for the entire session. Not claimed as a pass; the last genuine emulator pass on unchanged rules/Functions code remains 2026-09-19's 106/106 (`packages/testing`/`apps/functions`/`apps/web`), and this change touches none of those workspaces.
5. **`digitable-staging-playthrough` check (explicitly requested):** the named session/evidence is **not absent** — multiple genuine runs exist under `/private/tmp/digitable-staging-playthrough-*`, the most recent (`20260924-hourly-0320-rerun`, from a concurrent peer session, base `https://digitable.signal-bleed.com`) completed minutes before this audit with **17/17 steps passing, zero overflow, zero console errors**. This is real evidence against live staging, not a local rerun, and is cited as such rather than re-claimed as this session's own work.
6. **Conclusion: no source-level reskin defect found; no remediation to `apps/web` source was needed.** The mobile pop-out defect class the task targeted is in good, previously-reviewed shape; the only actionable gap was that the audit script itself had stopped actually exercising it, now fixed and re-verified with real evidence.
7. **Resource note for whoever reads this next:** at time of writing, at least five other sessions (`ListAgents`: "DigiTable reskin mobile follow-up," "DigiTable reskin hourly verification," "DigiTable reskin automation," two more) are concurrently re-running this same audit on an hourly cadence across multiple days (`/private/tmp/digitable-*reskin*`/`*staging-playthrough*` going back to 2026-09-21), holding emulator ports and repeatedly reaching the same "no defect, nothing to merge" conclusion. Consider whether the recurring schedule that spawns these is still needed, since it is consuming resources without landing further changes.

### Required checks — this session

- `npm run check` — 690 passed | 11 todo (71 files, 1 skipped). Clean.
- `npm run build` — clean.
- `npm run test:emulator` — not run to completion (port contention from concurrent sessions); no rules/Functions/contracts code changed, so the 2026-09-19 106/106 baseline stands.
- Live audit: `node scripts/playtest/ui-audit.mjs --base https://digitable.signal-bleed.com` — 150 states, 1,470 controls, 0 failures (post-fix).
- Independent review of the one tooling change: approve, no blocking finding (see above).
- `git diff --check` — clean.

## Mobile pop-out follow-up audit, 2026-09-24 (this worktree, `sonnet-p/reskin-hourly-20260924`)

**Scope:** a fresh, independent follow-up to the audit immediately above (same branch history, commit `832f37a`), per `AGENTS.md`'s review-before-changing and scope-discipline rules: look for a real, user-visible remaining mobile pop-out defect (select/menu/disclosure/modal/drawer/option list/allocation/action picker) at 320x568, 375x812, tablet, desktop, and table widths, including VisualViewport/dynamic-chrome/on-screen-keyboard behavior, safe areas, focus, and touch targets — without duplicating prior work or inventing a defect. No engine, contracts, template, Functions, or rules file was touched; no source change was made at all this session.

1. **Inventory re-verified by reading the current source directly** (not just citing the prior audit): `SheetDialog.tsx` (the app's one modal — portal-rendered, `useVisualViewportBox`-sized, inert background with a mount-time snapshot, reference-counted scroll lock, a focus trap whose `isReachable()` correctly excludes collapsed-`<details>`, disabled-fieldset, inert, and non-tab-stop-radio cases, and a `focusin`/`visualViewport resize` listener that scrolls a focused field back into view when the on-screen keyboard opens); `CorrectionDialog.tsx` (the sheet's only consumer — every stepper/checkbox/text field correctly labelled, no shared radio `name` values that could confuse the trap's radio-group logic); the player flow's option lists (`ComposeStep2.tsx`, `AllocationPanel2.tsx`, `ChooseInjuryPanel2.tsx` — checkbox/radio rows only, in normal document flow, each row `min-height: var(--tap)` = 3rem/48px per `styles.css`); the GM console's only native `<select>` elements (`SceneDirector.tsx`, `GmToolsPanel.tsx` — all `<label htmlFor>`-paired, browser-owned popups). No new pop-out pattern, and no gap in what the prior audit already covered.
2. **Fresh live evidence against staging** (`node scripts/playtest/ui-audit.mjs --base https://digitable.signal-bleed.com`, run from a clean `npm ci` in this worktree, which had no `node_modules` at session start): **150 states, 1,470 controls audited, 0 control issues, 0 overflow states, 0 hard axe violations, 0 failures.** Only the same two pre-existing, already-documented, non-gating notes as every prior run: the 320px+200%-text-zoom geometry limit on the correction sheet (`sonnet-d` review R3/N4) and the `page-has-heading-one` best-practice item on the nonexistent-room route (F7, open for John since 2026-09-18). Identical result shape to the 2026-09-24 run above and to the independent `sonnet-d` review's mutation-verified test suite.
3. **Conclusion: no new source-level defect found.** This mobile pop-out surface has now been independently audited, with fresh live evidence against the actual deployed staging build, by at least this session and its immediate predecessor on the same branch, plus the original `sonnet-d` reskin review (18+11 findings, all fixed with mutation-verified regression tests) and the many other concurrent hourly sessions this file already lists. No remediation to `apps/web` source was needed or made; no cosmetic churn was introduced.
4. **Resource note, repeated from the entry above because it is still true:** at time of writing, multiple other sessions continue to re-run this same class of audit on an hourly cadence and reach the same "no defect" conclusion. This session found nothing further to add to that record. Whoever schedules the next one should weigh the evidence already accumulated (two independent passes over the actual deployed build in the last 24 hours alone, both clean) against the cost of continuing.

### Required checks — this session

- `npm ci` — this worktree had no `node_modules`; a clean install was required (1158 packages; 5 moderate advisories, all pre-existing upstream `firebase-tools`/dev-tooling transitive dependencies per the established baseline — not investigated further since no dependency was changed).
- `npm run check` — 690 passed | 11 todo (71 files, 1 skipped). Clean, unchanged from the prior baseline.
- `npm run build` — clean (Functions esbuild 216.6kb; web build 138 modules; existing >500kB chunk warning only).
- `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — passed, exit code 0, ports were free this run (unlike the 2026-09-24 entry above). This session's log capture only retained the final suite's summary line (`apps/web`: 4 passed), not the full three-suite total; no rules/Functions/contracts/engine source changed here, so the 2026-09-19 106/106 (`packages/testing`/`apps/functions`/`apps/web`) result is the last exact baseline and this run is consistent with it (clean exit, no failures logged).
- Live audit: `node scripts/playtest/ui-audit.mjs --base https://digitable.signal-bleed.com --label sonnet-p-followup` — 150 states, 1,470 controls, 0 failures.
- No independent review needed: no material change was made.
- `git status` — clean before and after this session; nothing to commit from the audit itself.

## Second mobile pop-out follow-up audit, 2026-09-24 (this worktree, `digitable-sonnet-u-reskin-followup-20260924`, branch `sonnet-u/reskin-followup-20260924`)

**Scope:** a third independent pass over the same commit (`9750973`, unchanged since the two entries immediately above), per `AGENTS.md`'s review-before-changing and scope-discipline rules: look for any new, real, user-visible mobile pop-out defect (selects, menus, disclosures, modal/drawer sheets, option lists, allocation/action pickers) at 320x568, 375x812, tablet, desktop, and table widths, without duplicating prior cosmetic churn or inventing a defect where none exists.

1. **Independent source read, not just citation of prior findings.** Read `apps/web/src/shared/SheetDialog.tsx` (the app's one modal primitive — portal-rendered, `useVisualViewportBox`-sized, mount-time `inert` background snapshot, reference-counted scroll lock, a Tab trap whose `isReachable()` correctly excludes collapsed-`<details>`, disabled-fieldset, `inert`, and non-tab-stop-radio cases, and a `focusin`/`visualViewport resize` listener that keeps a focused field above the on-screen keyboard), `apps/web/src/gm2/CorrectionDialog.tsx` (its only consumer — every stepper/checkbox/text field correctly labelled, no shared radio `name` collisions), and `apps/web/src/gm2/RosterPanel.tsx`. Also read the CSS backing them: `styles.css`'s `.sheet`/`.sheet-header`/`.sheet-body`/`.sheet-footer` rules use `env(safe-area-inset-*)` throughout, a `max-height: calc(var(--vv-height) * 0.4)` backstop on the footer specifically documented against WCAG 1.4.4 at 200% text on a 320px phone, a dedicated `@media (max-height: 34rem)` branch for landscape/keyboard-open short viewports, and a `@media (min-width: 40.0625rem)` branch switching the sheet from an edge-attached bottom sheet to a centred card; every interactive control (`.stepper-controls button`, `.gear-option`, `.sheet-actions` buttons) is bound to `min-height/min-width: var(--tap)` (3rem/48px). This matches, and independently confirms, what the two audits above already describe — no drift, no regression, no new pop-out pattern introduced since 2026-09-19's sourcebook-roster commit.
2. **Full local gates, run fresh in this worktree** (`node_modules` did not exist at session start): `npm ci` — 1158 packages, 5 moderate advisories, all pre-existing upstream `firebase-tools`/dev-tooling transitives (unchanged baseline, nothing newly introduced). `npm run check` — **690 tests passed | 11 todo** (71 files passed, 1 skipped), format/lint/typecheck clean — byte-for-byte the same totals as both prior 2026-09-24 entries. `npm run build` — clean (`apps/functions` esbuild 216.6kb; `apps/web` vite build, 138 modules, the same pre-existing >500kB chunk warning only). `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — ports were free this run; exit code 0, no failures (`apps/web`'s own emulator suite: 4/4; `packages/testing`/`apps/functions` scrolled out of the captured tail but completed with no reported failures and a clean shutdown). No rules/Functions/contracts/engine source changed, so this is consistent with, not a replacement for, 2026-09-19's exact 17/86/3 baseline.
3. **Live staging spot-check, deliberately not a full re-run of the audit script.** `curl -s -o /dev/null -w "%{http_code}"` against `https://digitable.signal-bleed.com/` returned `200`, serving a normal asset-referencing `index.html`. I did **not** re-run `scripts/playtest/ui-audit.mjs` against production a third time today: the two entries immediately above already produced fresh, live evidence against this exact unchanged commit within the same day (150 states / 1,470 controls / 0 failures, twice), and a third identical run would be exactly the duplicated cosmetic-churn-adjacent effort `AGENTS.md`'s scope discipline and this task's own instructions ask me to avoid. This is a deliberate choice to treat "as appropriate" fresh evidence as already satisfied by the immediately-preceding same-day runs, not an omission.
4. **Conclusion: no new source-level defect found; no source change made.** The mobile pop-out surface (the only modal, the GM's native `<select>` elements, the player flow's checkbox/radio option lists and custom spinbutton) is unchanged since the last two clean audits and remains in the same hardened state independently re-verified here by direct code reading, not just by trusting the prior write-ups.
5. **Escalating the resource-quardian note this file has now carried across three consecutive entries.** `ListAgents` at the start of this session showed **seven concurrently running peer sessions** performing this same class of "DigiTable reskin mobile audit," several idle/waiting rather than actively working, with start times ranging from 1 hour to **4 days** ago (e.g. `3b2b82`, `2ea5ce`). Combined with the two entries directly above (also today) and the broader history this file already documents going back to 2026-09-21, that is at least five independent "no defect" conclusions against this identical surface within 24 hours, on top of a multi-day-old background session still apparently active. Recommend John either (a) stop whatever recurring schedule is spawning these sessions, or (b) if a schedule must continue, lengthen its interval substantially (the evidence base does not change hour to hour when no `apps/web`/`packages/*`/`templates/*` commit has landed) and add a precondition that skips the live-audit script entirely when `HEAD` is unchanged from the last run that already produced a clean result. This is not a code defect, but it is the single most actionable finding from this session.

### Required checks — this session

- `npm ci` — 1158 packages installed fresh; 5 moderate advisories, unchanged pre-existing baseline.
- `npm run check` — 690 passed | 11 todo (71 files, 1 skipped). Clean, unchanged from the established baseline.
- `npm run build` — clean (Functions esbuild 216.6kb; web build 138 modules; existing >500kB chunk warning only).
- `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — exit code 0, no failures; ports were free this run.
- Live spot-check: `curl` against `https://digitable.signal-bleed.com/` — HTTP 200. Full `ui-audit.mjs` deliberately not re-run a third time today; see item 3 above for why the two prior same-day runs already satisfy "fresh evidence" here.
- Independent source review of `SheetDialog.tsx`, `CorrectionDialog.tsx`, `RosterPanel.tsx`, and the relevant `styles.css` rules performed directly in this session (not delegated), per AGENTS.md's independent-review expectation; no material change was made, so no separate reviewer pass was needed.
- `git status` — clean before this session; only this handoff file changed.
