...
- **Status:** The six playable character sheets now match the owner-supplied sourcebook roster (Iryna, Nicole, Cosgrave, Chuck, Astrid, and Flint), including equipment, abilities, advances, injuries, Last Stands, ability bonuses, and typed utility-item behavior. Commit `5e8907b` is deployed to Firebase Hosting and all five callable Functions at <https://digitable.signal-bleed.com>. A fresh-room live smoke passed all 17 GM/player/table steps after the Functions update, and a new deployed-session rerun passed on 2026-09-19. Physical-device evidence remains open for John. A final mobile pop-out/a11y audit (2026-09-20, branch `sonnet-h/reskin-final-mobile-audit`, not deployed) found and fixed three narrow defects plus a stale audit script; see "Final mobile audit (sonnet-h)" below.
...
- **Last updated:** 2026-09-20 by Claude (Sonnet 5) after the final mobile audit on `sonnet-h/reskin-final-mobile-audit`; previously 2026-09-19 by Codex after independently rerunning the deployed GM/player/table smoke
...
- **Next action:** John conducts the complete flow on two physical devices, including a lost-identity recovery attempt from a fresh/private browser, then records device/browser evidence. Do not promote to a separate production Firebase project without explicit direction.
