...
- **Fresh hourly verification (2026-09-25):** the missing `digitable-staging-playthrough` session was again not treated as completion. On the clean reviewed reskin candidate (`c220dd2`), `npm run check` passed (**692** tests, **11** todo; 71 files passed and 1 skipped), `npm run build` passed (existing Vite large-chunk warning only), and the complete emulator suite passed (**18** rules/testing, **86** Functions, **4** web tests). The emulator emitted only the known Admin-SDK metadata lookup warnings under its isolated environment. `node scripts/playtest/two-device-smoke.mjs --base https://digitable.signal-bleed.com --out /private/tmp/digitable-staging-playthrough-20260925-0124 --reload --port 9510` then passed all **17/17** GM/player/table steps, with zero console errors, failed requests, or positive horizontal overflow at 375/768/1024/1280/1920 px; its 21 screenshots and report remain in that directory. This verifies the deployed pre-reskin staging build and does not deploy or merge the reskin candidate.
