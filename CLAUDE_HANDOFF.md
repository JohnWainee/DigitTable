...
- **Fresh staging rerun (2026-09-23 HST, automation follow-up):** the expected `digitable-staging-playthrough` session is still absent, so the deployed build was exercised directly once more from this isolated candidate: `node scripts/playtest/two-device-smoke.mjs --base https://digitable.signal-bleed.com --out /private/tmp/digitable-staging-playthrough-20260923-automation-final --reload --port 9463`. All **17/17** GM/player/table steps passed in 37 seconds (06:49:27Z–06:50:03Z): fresh room creation, isolated player/table admission, character claim, opposed action, pause/resume, scene advance, direct resume, reload recovery, and responsive sweeps. All three devices reported zero console errors and failed requests; responsive overflow was 0 px at every tested GM/player width and 0/-15 px for the table at 375/768/1024/1280/1920 px (the -15 px value is spare viewport width). The report and 21 screenshots remain at `/private/tmp/digitable-staging-playthrough-20260923-automation-final/`. This validates the deployed pre-reskin build only; no build was deployed or merged.
...
### Final verification pass (sonnet-k, 2026-09-22)

Branch `sonnet-k/reskin-final-verification-20260923`, isolated worktree, candidate `eedab53` (reskin/hardening source unchanged since `c180820`; everything after is docs-only staging-smoke recording). Record: [`docs/reviews/2026-09-22-sonnet-k-final-verification-review.md`](docs/reviews/2026-09-22-sonnet-k-final-verification-review.md). No code changed; not merged, not deployed.

- **Question:** an independent source-level inspection of every popup/sheet/dropdown/dialog and its dynamic mobile keyboard/browser-chrome behavior, looking specifically for a regression the prior mobile-audit lanes missed. **Answer:** none found. Re-confirmed the inventory is still exactly `SheetDialog` (one consumer: `CorrectionDialog`) plus six native `<select>`s and one in-flow `<details>` — no new pop-out surface exists. Worked through `useVisualViewportBox`'s `position: fixed` + visual-viewport-offset compensation by hand against the existing pinch-zoom test case and found the coordinate math correct (no double-scaling under pinch-zoom). Confirmed the `html.sheet-open { overflow: hidden }` root-scroll lock is sound given every other `<body>` child is also made `inert` while a sheet is open (background is excluded from hit-testing, so there is no scrollable ancestor a touch could reach). Confirmed the global 16px/44px minimum rule covers every text input, select, and textarea app-wide, not just inside the sheet.
- **Commands and results:** `npm run check` exit 0 (**708 tests passed, 11 todo**, 75 files passed, 1 skipped — matches the figure already on record for this commit). `npx vitest run apps/web/test/shared/SheetDialog.test.tsx` — 22/22 passed.
- **Residual (unchanged):** the physical iOS Safari pass (visual-viewport-only keyboard shrink, VoiceOver) remains the one thing no local/headless lane, including this one, can produce — `scripts/playtest/ui-audit.mjs` documents this as a known, not hidden, limit.
