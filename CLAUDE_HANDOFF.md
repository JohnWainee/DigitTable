...
**Is fresh live staging/ui-audit evidence required? Yes.** `scripts/playtest/ui-audit.mjs`'s last recorded run (`docs/evidence/sonnet-d-reskin/`, and the "staging-consolidated" run noted under "Claude takeover checkpoint" above) predates both `5e8907b` and `4083123` — it never saw the "Recover your seat" form, the "Destroy Cowboy hat" button, or the "Mark and regain Blood" button, so no real-browser geometry evidence exists for any of them, fixed or not. jsdom (what `npm run check` exercises) does no layout, so it cannot confirm the fixed buttons actually measure >=44px in a real engine, and cannot confirm the new recovery form introduces no overflow/clipping at phone widths. Rerun `node scripts/playtest/ui-audit.mjs --base <local emulator-served build or staging> --label post-utility-item-fix --out <path>` before considering this closed, and before any Hosting redeploy.
