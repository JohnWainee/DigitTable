...
Merge remains John's decision. **Independent review of the integration itself** is recorded in [`docs/reviews/2026-09-16-issue-14-integration-independent-review.md`](docs/reviews/2026-09-16-issue-14-integration-independent-review.md): approved, no blocking findings, nothing lost in the merges, every conflict resolution and baseline-relative assertion checked against the engine's sequencing. Its one Low finding (the game-command logging test's title promised rejected-path coverage its body never exercised, a shape inherited from A04) was fixed: the test now submits an accepted and a rejected command and asserts neither's payload reaches the structured log. Re-verified after the fix: `npm run test:emulator` 108/108, static checks clean.
...
### B02 — Characters and resources — DONE (branch `sonnet-b/b02-characters`)

- **Branch:** `sonnet-b/b02-characters`, branched from `sonnet-b/etr-rules` (B01), which is not yet merged to `main`. Also carries a cherry-pick of Sonnet A's `sonnet-a/errors-for-b` commit (`contracts: add Eat the Reich character/round stable error codes for B02/B03`), needed for `CHARACTER_TAKEN`/`CHARACTER_RETIRED`/`INSUFFICIENT_BLOOD` — not yet on `main` either.
- **Deliverable:** a full structural rewrite of `templates/eat-the-reich`'s state/commands/events/view/schemas/engine for `schemaVersion: 2` (docs/ETR_RULES_IMPLEMENTATION_PLAN.md §1's fresh-start decision), replacing the old single-stat/wound-counter placeholder:
  - `src/state.ts`: the seven `Stat`s, `ItemState` (uses/bonus), typed `AbilityState`/`AbilityEffect` (Appendix B vocabulary), typed `InjuryPenaltyTag` (six tags), two-box `InjuryCategoryState`, `AdvanceState`, `LastStandState`, and `CharacterState` keyed by roster id with `claimedByMemberId: MemberId | null`, `blood: 0..10`, `downed`/`retired`.
  - `src/roster.ts`: `ORIGINAL_ROSTER`, the six original characters from `docs/ETR_RULES_MATRIX.md` Appendix A (matching stats/items exactly against `docs/ETR_PLAYTEST.md`'s S01/S02 fixture baseline for Rook and Tallow), each with 3 abilities (special/blood/other), 3 advances, 3 injury categories (typed second-box penalties spanning all eight Appendix B tags for coverage), a Last Stand.
  - `src/commands.ts`/`src/events.ts`: `ClaimCharacter`, `ReleaseCharacter`, `HealInjury` (and their events). The full declare/review/roll/allocate loop is explicitly B03's, not this slice's — matches the matrix's own P1-P3/P7 (B02) vs. P4-P6/D/O/A/I (B03) split.
  - `src/pool.ts`: `buildPool`, a pure function computing base+item+ability dice with the four enforceable injury tags applied (`oneItemPerTurn`, `statDelta`, `allStatsDelta`, `noBloodSpend`) — the matrix's B02-scoped P1-P3/P7 test cases. `noBonusDice`/`noSpecials` are typed and parsed but have nothing to enforce yet (bonus claims and SPECIAL activation don't exist until B03) — left as explicit `test.todo`s, not silently skipped.
  - `src/view.ts`: `CharacterPartySummary` (public to every viewer, all roster entries) vs. `CharacterFullSheet` (self + GM only) — replaces the old flat character/threat/roll view.
  - `src/schemas.ts`/`src/engine.ts`: hand-rolled parse functions for the new shapes; `authorizeGameAction`/`decide`/`reduce`/`project` for the three commands; `explainPool` adapts the still-old-shaped `PoolInput` (`actionId`→stat, `gearIds`→itemIds) pending a contract proposal to extend it (see below); `validAllocations` returns `[]` (no roll concept exists yet); `migrate` fails closed on anything but `schemaVersion 2`.
  - Deleted `src/content.ts` and `src/allocations.ts` (the old placeholder's single hardcoded location/objective/threat/character and its two-option allocation catalog) — fully superseded, not extended, per the matrix's explicit "do not switch 5+ to 4+ and call it fidelity" instruction.
  - All 13 existing test files rewritten for the new shapes (same file names, same invariants under test — projection isolation, size budgets, schema round-trips, authorization, retry idempotency — against the new commands); `templates/eat-the-reich/test/allocations.test.ts` deleted (no allocation catalog exists until B03); added `templates/eat-the-reich/test/roster.test.ts` (matrix C2/C3: six unique characters, each matching the custom-character shape) and converted the B02 rows of `templates/eat-the-reich/test/rulesMatrix.todo.test.ts` from `test.todo` to real assertions (some rows correctly remain `.todo`, annotated with why — e.g. item/ability use isn't actually *charged* by any command until B03).
- **Second contract proposal to Sonnet A (not yet posted as its own issue comment; noted here and in the B02 issue status comment):** `PoolInput` (`packages/contracts/src/template.ts`) is still shaped `{actionId, gearIds}` from the old placeholder. `explainPool`'s doc comment in `src/engine.ts` describes the interim adapter (actionId doubles as stat name, gearIds as itemIds; ability-die/bonus-claim preview is not representable yet). Proposed extension: either make `PoolInput` a `TPoolInput` generic per template, or add optional `stat?`, `abilityIds?`, `bonusClaimIds?` fields. Not blocking — B03's actual dice-rolling `decide` is unconstrained by `PoolInput` and will compute the real pool in full regardless.
- **Verification performed** (all in `.claude/worktrees/sonnet-b-rules`):
  ```
  ...
  npx vitest run templates/eat-the-reich                             # pass: 87 passed, 52 todo, 13 files (1 skipped file unrelated)
  ...
  npx vitest run --project '!web'                                    # pass: 117 passed, 52 todo, across 20 files
  ```
  **Expected, out-of-scope breakage** (not fixed here — `apps/*` is explicitly Sonnet C's/A's territory per the board's collision rules, and this slice's whole point is the structural rewrite the matrix mandated): `npm run lint`, `npm run typecheck`, and `npm run build` at the repo root all fail, entirely inside `apps/web` (173 lint errors, ~50 typecheck errors, a Vite `MISSING_EXPORT` build failure) — every one of them is `apps/web` importing old template exports/view fields (`MAX_PUSH_DICE`, `ACTION_ID`, `THREAT_ID`, `view.threats`, `view.activeRoll`, the old `BeginAction`/`SubmitOpposition` commands) that no longer exist post-rewrite. No file under `packages/*` or `templates/eat-the-reich` has any lint/typecheck error. C01/C02/C03/C04 are the tracked follow-up to migrate `apps/web` onto the new B02 (and later B03/B04) contract.
- **Dependencies/next:** B03 (the resolution loop: `BeginAction`/`ReviewAction`/`AllocateResults`/`ChooseInjuryCategory`, die interpretation, attack pool, the five-family allocation catalog, injury rolls) branches from `sonnet-b/b02-characters` next, per `docs/ETR_RULES_IMPLEMENTATION_PLAN.md` §3. Rebase both `sonnet-b/etr-rules` and `sonnet-b/b02-characters` onto `main` once PR #17 (B01) and Sonnet A's `errors-for-b` land, so the cherry-picked commit collapses into a normal ancestor rather than staying a cherry-pick.

### B03 — Correct resolution loop — DONE (branch `sonnet-b/b03-resolution-loop`)

- **Branch:** `sonnet-b/b03-resolution-loop`, stacked on `sonnet-b/b02-characters` (which stacks on `sonnet-b/etr-rules`). `schemaVersion` bumped 2 -> 3 (same fresh-start rationale as B02, `docs/ETR_RULES_IMPLEMENTATION_PLAN.md` §1: no live room has ever existed under any prior shape); `templateVersion` 0.2.0 -> 0.3.0.
- **Deliverable:** the full declare -> GM review -> single server roll -> allocate loop (`docs/ETR_SESSION_FLOW.md` §6), replacing the placeholder's `BeginAction`/`SubmitOpposition`/`AllocateResults` shape entirely:
  - New commands: `BeginAction` (declare: stat/items/abilities/bonus claims/engaged threats/note, no resources charged yet), `ReviewAction` (GM-only: approves/strikes bonus claims, confirms engaged threats, server rolls player pool **and** Attack pool from the same seed, charges Blood/item uses, applies passive D5 triggers), `AllocateResults` (player spends kept dice against the five allocation families with Challenge absorption, rolls the injury category die if the GM has remaining successes), `ChooseInjuryCategory` (the I1 "category full" overflow follow-up).
  - `src/state.ts` gains `ObjectiveState`, `ThreatState`/`ThreatFlags` (typed: `discardBelow`, `noFeeding`, `attackCritOnSix`, `challengeLocked`, `injuryMarksWholeCategory`), `RollRecord` (the full declare/review/allocate lifecycle), `KeptDie`, `BonusClaimRecord`, `InjuryChoicePending`. No Scene/round wrapper yet (B04) — a flat `objectives`/`threats` map, matching the matrix's own B03-vs-B04 split.
  - `src/pool.ts` adds `interpretDie`/`pointsForResult` (D1), `interpretAttackDie` (D3), `isLastUse`/`lastUseBonusDice` (P5). `src/resolution.ts` (new file, mirrors the `pool.ts`/`allocations.ts` pure-function split): `effectiveAttack`/`computeAttackDiceCount` (O1-O3), `rollCategoryIndex` (I1), `challengeDamage` (A7), `groupAllocationsByTarget` (A7-A8). `src/allocations.ts` replaced with the new `AllocationTarget` union (objective/threat/defend/feed/special).
  - `src/view.ts` adds `ObjectiveView`, `ThreatPublicView`/`ThreatGmView` (unrevealed Threats hidden from non-GM projections), `RollViewActing`/`RollViewFull` (a declared roll shows only "{character} is acting" to non-owner/non-GM viewers; once rolled, matrix §6.2's "nothing hidden here in ETR" applies and every viewer sees full dice detail; resolved rolls drop out of the live projection — their effects already live in `roster`/`objectives`/`threats`).
  - Every P0 matrix row in 3.2 (P4-P5), 3.3 (D1-D3), 3.4 (O1-O4), 3.5 (A1-A9, including the p.38 Challenge worked example), and 3.6 (I1, I2, I5) is implemented and tested. P1 items handled per the matrix's own manual fallback, not silently dropped: Flashback (D4), late bonus dice (P6), secondary-objective rewards (S2), Loot (S9), and Last Stand (I3) are deferred with their book-specified manual GM-correction fallback noted in `test/rulesMatrix.todo.test.ts`; D5 passive triggers are implemented and unit-tested against a synthetic character (no roster character currently carries a passive ability to exercise it live); I4 (`injuryMarksWholeCategory`) is implemented in the injury-marking logic.
  - `explainPool`'s `PoolInput` adapter gap and a matching **third contract proposal** are already recorded in `docs/ETR_RULES_IMPLEMENTATION_PLAN.md` §7 / the B02 issue comment (extend `PoolInput` with `abilityIds`/`bonusClaimIds` — not blocking, `decide`'s real pool computation is unconstrained by it).
  - All prior tests still pass; new tests: `test/resolution.test.ts` (pure O1-O4/A7/I1 helpers, including the p.38 worked examples verbatim), `test/decideResolution.test.ts` (29 tests: BeginAction/ReviewAction/AllocateResults/ChooseInjuryCategory decide-level, fixed-face `FixedSequenceRandom`), extensions to `test/pool.test.ts` (D1-D3, P5), `test/reduce.test.ts` (item-use/Blood deltas from `ActionRolled`), `test/project.test.ts` (declared-roll redaction, unrevealed-Threat hiding, resolved-roll pruning), `test/lifecycle.integration.test.ts` (one full claim->declare->review->allocate loop through `runCommand`/`projectViewer`, plus a `BeginAction` retry-idempotency test), `test/budgets.fixture.test.ts` (worst case now includes 3 Objectives, 3 Threats with every flag set, and 6 concurrent in-progress rolls — still well under budget).
- **Verification performed** (`.claude/worktrees/sonnet-b-rules`):
  ```
  npm run typecheck --workspace @digitable/template-eat-the-reich   # pass
  npx eslint templates/eat-the-reich                                 # pass, 0 errors, 0 warnings
  npx vitest run templates/eat-the-reich                             # pass: 163 passed, 19 todo, 15 files (1 skipped file unrelated)
  npm run format                                                      # pass (whole repo)
  npm run typecheck --workspace @digitable/contracts --workspace @digitable/engine --workspace @digitable/testing   # pass
  npx vitest run --project '!web'                                    # pass: 193 passed, 19 todo, 22 files
  ```
...
### B05 — Session fixture and review — DONE (branch `sonnet-b/b05-fixtures-review`)

- **Branch:** `sonnet-b/b05-fixtures-review`, stacked on `sonnet-b/b04-scenes-rounds`. No schema bump this slice (content/fixtures only, plus one GM-only `ThreatState.notes` field — additive, not a version-breaking reshape by itself, but ships in the same PR as the schema-affecting B04 work already on this branch lineage).
- **Deliverable:**
  - **`ThreatState`/`SceneThreatInput` gain `notes: string`** (GM-only foreshadowing text, matrix Appendix C: "Foreshadowing lines for elites live in the GM-only notes field and never project to players or the table") — threaded through `ThreatGmView` (present) vs `ThreatPublicView` (absent), schemas, and a new isolation test. This was a real gap in B03/B04 caught while building the Appendix C fixture, not something deferred on purpose.
  - **`src/scenes.ts`**: `ORIGINAL_MISSION`, the four original scenes from `docs/ETR_RULES_MATRIX.md` Appendix C (drop-forecourt, metro-platform, printworks, signal-mast) as `SceneDefinition`s — the exact payload shape `LoadScene`/`NextScene` expect. The Enforcer and The Warden (both elite) load unrevealed with a GM-only foreshadowing note; the GM `RevealThreat`s them per the book's pacing. No server-side scene catalog exists in the template — the GM console (Sonnet C) looks a scene up by id from this list and sends it as the command payload; documented as a deliberate design choice in the module's own comment.
  - **`src/privateContent.ts`**: `loadPrivateContentPack`/`parsePrivateSceneDefinition` — a GM's own private, git-ignored `content/private/*.json` scenes (matrix §5, `.gitignore` entry already added in B01). **Deliberately not exported from `./index.js`**: it uses `node:fs`/`node:path`, and the template's barrel export is what `apps/web`'s browser bundle pulls from (AGENTS.md: the template must build and run without `apps/web` or Firebase present) — bundling `node:fs` into a browser build would break it. Import it directly by path from Node-side tooling only (a CLI script, or a trusted Functions-side loader); a browser file-picker needs a different mechanism (File System Access API / `<input type="file">`, Sonnet C's territory) and can call the exported `parsePrivateSceneDefinition` directly on already-parsed JSON without the filesystem dependency.
  - **`test/playtest.test.ts`**: `docs/ETR_PLAYTEST.md` (F04)'s S01, S02 (x2), S03, S04, S05, S08, S09, S10 bound as fixed-seed tests against the *real shipped* `ORIGINAL_MISSION` content via `runCommand`/`projectViewer` (not synthetic fixtures) — "where cheap" per the coordinator's direction. S06 (disconnect mid-roll) and S07's specific "event tail may be empty" proof need live client/reconnect/event-store infrastructure that doesn't exist at the template level; explicitly not implemented here, noted as Sonnet A's A05-A07 territory in both the test file's own header comment and `test/rulesMatrix.todo.test.ts`.
  - **Real bug found and fixed by S05's test**: `decideAllocateResults`'s O4 zero-success Attack bump and the SPECIAL-effect threat-delta application both read `ctx.state.threats[primaryEngagedThreatId]` with a non-null assertion, assuming the primary engaged Threat still exists. If a GM `EditScene`s a Threat out of existence between `ReviewAction` and the player's `AllocateResults` (exactly matrix S05's scenario), this crashed the transaction rather than failing closed. Fixed: both call sites now check the Threat still exists first and simply skip the effect if not — matches matrix S05's explicit requirement ("never silently applied to a missing threat, never crashes the transaction"). This is exactly the kind of finding adversarial scenario tests are for, not a should-have-been-caught-earlier gap in isolation testing (the happy-path unit tests for O4 and SPECIALs, written in B03, never exercised a concurrently-edited scene).
  - `test/scenes.test.ts` (8 tests): shape/uniqueness/rating-band checks on `ORIGINAL_MISSION`. `test/privateContent.test.ts` (7 tests): loader behavior against real throwaway temp directories (never this repo's own `content/private/`), malformed-file/invalid-JSON fail-closed behavior, missing-directory-returns-empty behavior.
  - Budget/migration/projection-isolation closure: a new cross-cutting test in `playtest.test.ts` loads every `ORIGINAL_MISSION` scene through a real `LoadScene` and checks `checkAuthorityBudget`/`checkProjectionBudget` for every viewer; the B02/B03/B04 migration-fail-closed tests (v1 through v3, all reject) and the character-sheet/roll/threat isolation property tests from earlier slices are unchanged and still pass.
- **Verification performed** (`.claude/worktrees/sonnet-b-rules`):
  ```
  npm run typecheck --workspace @digitable/template-eat-the-reich   # pass
  npx eslint templates/eat-the-reich                                 # pass, 0 errors, 0 warnings
  npx vitest run templates/eat-the-reich                             # pass: 216 passed, 10 todo, 19 files (1 skipped file unrelated)
  npm run format                                                      # pass (whole repo)
  npm run typecheck --workspace @digitable/contracts --workspace @digitable/engine --workspace @digitable/testing   # pass
  npx vitest run --project '!web'                                    # pass: 246 passed, 10 todo, 26 files
  ```
  Same expected, out-of-scope `apps/web` breakage as B02-B04 (Sonnet C's territory).
- **Independent rules review:** recorded in `docs/reviews/2026-09-17-etr-rules-independent-review.md` — a fresh reviewer subagent (no prior context, given only the branch and the specification docs, per the board's requirement that the author cannot self-certify) checked every matrix row 3.1-3.8 against the actual code and tests, independently re-derived the p.38 worked examples by hand, traced the claimed O4/SPECIAL bug fix in the diff, and specifically probed projection isolation. Verdict: the core rules engine and manual-adjudication register are correct and honest, but the review found **1 Critical finding**: `LoadScene`/`NextScene`/`EditScene` were broadcasting the complete `ThreatState` — including GM-only foreshadowing `notes` and unrevealed-Threat existence — to the shared event destination every player and the table can read, even though `project()` itself redacted this correctly. This is exactly the kind of gap the review was asked to probe, and no prior test caught it (every isolation test asserted on `project()` output, never on raw event `effects`). Also 2 Low findings (an undocumented C5/Blood-sharing P1 deferral; a genuine spec tension between two Fable documents over whether a roll's free-text note stays GM-only or goes fully public once rolled).
  - **All three findings resolved in the same slice**, not deferred: the Critical finding is fixed (`decideSceneTransition`/`decideEditScene` now build explicit per-destination `effects` — full `ThreatState[]` to `gm`, a new `redactThreatsForShared()`-filtered copy to `shared` — mirroring the `ActionDeclared` pattern that already existed elsewhere in this file), with two new regression tests asserting directly on `decision.events[0].effects` so this class of gap can't recur silently. The C5 gap is now recorded in `test/rulesMatrix.todo.test.ts`. The spec-tension finding is documented in `docs/ETR_RULES_IMPLEMENTATION_PLAN.md` §6 for Fable/John to resolve explicitly (not code-changed, since the review itself called the current behavior defensible). Re-verified after the fixes: template package tests **218 passed, 11 todo, 19 files** (up from the review's own independently-reproduced 216/10); repo minus `apps/web` **248 passed, 11 todo, 26 files**.
- **Dependencies/next:** B01-B05 are all done on this stacked branch lineage (`sonnet-b/etr-rules` -> `b02-characters` -> `b03-resolution-loop` -> `b04-scenes-rounds` -> `b05-fixtures-review`). Remaining open items are the contract proposals to Sonnet A (see the "Contract proposals" section of each slice's PR/issue comment) and the `apps/web` integration Sonnet C already flagged watching for.
