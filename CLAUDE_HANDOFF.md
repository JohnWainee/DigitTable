...
- **Last updated:** 2026-09-18 by Claude (Sonnet 5, board role "D"): ink-black reskin and mobile pop-outs on branch `sonnet-d/reskin-mobile-sheets` (see "Ink-black reskin and mobile pop-outs" below and [`docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md`](docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md)). Before that: independent staging playtest ([`docs/reviews/2026-09-18-staging-independent-playtest-review.md`](docs/reviews/2026-09-18-staging-independent-playtest-review.md)).
...
- **Independent staging playtest (2026-09-18, Claude): pass.** The deployed build completed the full GM/player/table flow with no console errors, failed requests, missing art, or overflow in the smoke sweep, and recovered from reloads of all three surfaces, including mid-declaration and mid-allocation. It found one responsive defect, F1: the GM console overflows by 40px at 375px once the opening scene is loaded (an over-wide `#edit-target` select in a `<fieldset>`). The fix (`fieldset { min-width: 0 }`, `select { max-width: 100% }` in `apps/web/src/styles.css`, plus a smoke step that catches it) is committed on this branch and verified against the staging backend from a local build, but **staging Hosting has not been redeployed and still serves the unfixed build.** Open, non-blocking findings F2-F7 (no recovery-code entry UI though the UI tells users to enter one; recovery codes persisted in `localStorage`; a "Local fixture" document title on the live build; and copy, resume-flow, and heading nits) are in the review.
- **Reskin (sonnet-d), ready for John's review, not merged, not deployed:** `sonnet-d/reskin-mobile-sheets` (from `59c4fe3`) restyles every screen in `apps/web` (near-black ink surfaces, original acid/riot/cyan/pink accents, procedural print grain) and replaces the correction modal with a shared, visual-viewport-aware bottom sheet. Presentation only: no engine, contract, template, Functions, rules, dependency, or secret change. **Staging still serves the pre-reskin build**; it needs a later, explicitly authorized Hosting redeploy to show any of it. Full record below.
- **Next action:** (1) John decides whether to redeploy Hosting (`docs/RUNBOOK.md` section 3, step 4) to pick up F1, and which of F2-F7 to schedule; F3 is the one with security relevance. (2) Conduct the same flow on two physical devices (one GM, one player; the GM device may open the table view in a second tab if a third display is unavailable), record any usability defects, then promote through the normal PR/merge decision. Do not create a production Firebase project until that decision is explicit.
...
## Ink-black reskin and mobile pop-outs (board role D) — READY FOR JOHN'S REVIEW

Branch `sonnet-d/reskin-mobile-sheets`, worktree `.claude/worktrees/sonnet-d-reskin`, from `factory/today-integration` at `59c4fe3`. Commits, in order: `9939b7d` (reskin + `SheetDialog`), `c70333b` (fixes from the first independent review), `d96ae0c` (fixes from the second), `32d0397` (second-review nits), then a docs/evidence commit (this file, the review record, `docs/evidence/sonnet-d-reskin/`). Not merged, not deployed, no production resource, no secret, no dependency or lockfile change. Scope stayed inside `apps/web`, `scripts/playtest/ui-audit.mjs`, one provenance README section and docs.

### What changed

1. **Visual system** (`apps/web/src/styles.css`, a full rewrite of the same class names): near-black ink surfaces, original accents (`--acid` yellow, `--riot` red, `--cyan`, `--pink`, `--volt`) chosen to sit with the existing art pack's crimson/cyan/amber, condensed uppercase display type and mono labels from **system font stacks only**, hard-shadow "misregistered print" panels, masking-tape strips, torn art edges, hazard-stripe fixture banner, procedural print grain and halftone (one inline SVG `feTurbulence` tile and CSS gradients; **no new image file**, provenance in `assets/generated/eat-the-reich/README.md`, "UI reskin textures"). Palette contrast is asserted numerically in a test.
2. **Every control is a 48 px target with 16 px text-entry type**: buttons, the six native selects (kept native on purpose: the browser draws the option list, so it can never clip; we own the closed control and it cannot widen the page), text/number inputs, drawn checkbox/radio rows (the whole label is the tap target), the "Why?" summary, steppers. The one 26 px checkbox on the create screen (`#wrote-down`) is now a full-row label.
3. **`SheetDialog`** (`apps/web/src/shared/SheetDialog.tsx`, `useVisualViewportBox.ts`) replaces the ad hoc `.modal` in `CorrectionDialog` (its change detection and `onApply(patch, reason)` are byte-identical). Portaled to `<body>` with everything else `inert`; root scroll locked (reference counted); sized from `window.visualViewport` (`--vv-*`) so it stays inside what is visible with an on-screen keyboard, zoom or dynamic browser chrome; safe-area padding on all sides; pinned title and action row with an internally scrolling body; bottom sheet below 641 px, centred card above; a focused text field is scrolled back into view; focus trapped over *reachable* controls only and restored to the trigger; Escape closes; entrance motion exists only under `prefers-reduced-motion: no-preference`; never restricts pinch-zoom or one-finger panning.
4. `index.html`: `viewport-fit=cover, interactive-widget=resizes-content`, `color-scheme` and `theme-color` for the dark theme. Page zoom is never disabled.

### Evidence

- **Screenshots and machine-readable results:** [`docs/evidence/sonnet-d-reskin/`](docs/evidence/sonnet-d-reskin/README.md): `before/` and `after/` at phone 375, tablet 768, desktop 1280 and table 1920 for 12 real-flow states, plus every pop-out scenario shot and `report.json` for both runs. Produced by `node scripts/playtest/ui-audit.mjs` against the **local** emulators (`demo-digitable`) and local `vite preview` builds; staging was not touched.
- **Real-browser audit, 150 state × viewport captures per run (six viewports incl. 320×568 and 812×375):** before 1,458 controls audited with **210** findings (138 on the six selects at 13.3 px type and ~19 px tall, 54 on inputs and the checkbox, 18 on the "Why?" summary), **4** axe hard violations (real colour contrast, `.die-chip--discard` at 2.71:1), **267** audit failures, and a correction modal that was taller than the screen at every size with its title and actions unreachable, no scroll lock, no inert background, and failing with a keyboard, zoom and safe-area emulation. After: 1,518 controls, **0** findings, **0** axe hard violations, **0** horizontal overflow, **0** failures; all sheet scenarios pass (containment at six sizes, emulated keyboard, emulated zoom, a real two-finger pinch, safe-area at a notch-constrained 667×375, 150%/200% text, reduced motion on/off with a positive control).
- The audit fails (not skips) when a scenario throws or runs no checks; see the script header for its one documented limit.

### Required checks, run on the final code (`32d0397`), exit codes checked

- `npm run check` (format, lint zero warnings, typecheck, tests): **exit 0**, **653 passed | 11 todo** (64 files passed, 1 skipped). Baseline was 589 passed | 11 todo; the increase is 64 new tests: `SheetDialog` (22) and the CSS contract (42).
- `npm run build`: **exit 0** (existing large-chunk warning unchanged).
- `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator`: **exit 0**, 18 rules, 86 Functions, 3 web, identical to baseline.
- `node scripts/playtest/two-device-smoke.mjs` ran green (all steps) against the first reskin build; it was not re-run on the final build. The `ui-audit.mjs` flow performs the same create → join → claim → table → declare → roll → allocate → resolve → pause → advance steps and passed on the final build.
- Not run: `npm audit`, any staging session, any physical device.

### Independent reviews

Two fresh Opus review agents, read-only, mutation-testing copies: [`docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md`](docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md). First pass **changes required** (2 High: `touch-action: none` on the backdrop disabled pinch-zoom while a sheet was open, and the wide-viewport padding shorthand dropped the notch insets; 3 Medium; process gap), all fixed. Second pass found one blocker of mine (a lint error, so `npm run check` failed at `c70333b`; my earlier check had grepped ANSI-coloured output and missed it), a real regression from the first fix (`touch-action: pinch-zoom` forbids one-finger panning of a zoomed page), and test gaps; verdict **approve with nits** after the fixes, nits fixed in `32d0397`.

### Remaining limitations (nothing hidden)

1. **iOS Safari's keyboard case** (visual viewport shrinks, layout viewport does not) cannot be produced by headless Chrome; the hook is covered in jsdom and its use of `--vv-*` in real Chrome only through pinch-zoom. **A physical iPhone and Android pass (keyboard, notch, browser chrome) is still owed**, folded into the open two-device rehearsal (`docs/PLAYTEST_TWO_DEVICE.md`, `docs/evidence/today-qwen/CHECKLIST.md`).
2. One Chrome on one machine: no Safari, Firefox, screen reader (VoiceOver/NVDA) or Windows High Contrast run (a `forced-colors` block exists, untested).
3. **320 px with 200% text-only scaling**: the unchanged rem-padded console panels overflow the page (`dialogInsideViewport`/`noPageOverflow` recorded non-gating for that one scenario; the sheet's own checks still gate). Browser zoom is unaffected.
4. **`page-has-heading-one`** (axe best practice) remains on the player dashboard and the signed-out messages: staging finding **F7**, deliberately not fixed here. F2-F6 are also untouched.
5. Native `<select>` option lists cannot be screenshotted headlessly, so the popup itself is not pictured (it is browser-drawn by design).
6. The sheet's inert set is a mount-time snapshot (there is one dialog and no dynamic body portals); positive `tabindex` order is not modelled. Both are noted in the component.
7. The `h1` tilt, tape strips and clip-path art edges are decoration only. The tape strips are removed where multi-column layout could fragment them into the wrong column (`.table-screen`, and `.gm-screen` at wide widths), and the tilt is disabled under `prefers-reduced-motion: reduce`.

### Recommended integration step

1. Review `docs/evidence/sonnet-d-reskin/` (start with the README tables and the `after/` phone and `gm-correction-*` shots).
2. Open a PR from `sonnet-d/reskin-mobile-sheets` into `factory/today-integration` (presentation only; `apps/web/src/styles.css` is the one file other lanes could conflict with) and merge if satisfied. Merge remains John's decision; nothing was merged or opened here.
3. Then, with explicit authorization, redeploy staging Hosting (`docs/RUNBOOK.md` section 3, step 4). That also ships the earlier GM-overflow fix (F1). **Staging has not changed and shows none of this until then.**
4. Run the physical-device rehearsal on the redeployed build and record it in `docs/evidence/today-qwen/CHECKLIST.md`, paying attention to the on-screen keyboard over the correction sheet and notch/home-indicator clearance.
