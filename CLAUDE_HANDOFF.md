...
- **Phase 2 PR 3 is in progress on `worktree-phase2-pr3`.** Typed section-8 document contracts, platform-owned admission/GM-seat command inputs, admission error codes, and an environment-scoped staging Firebase anonymous-auth seam are implemented. Firebase CLI authentication verified access to `powerglove-1cd23` and its `digitable-staging-web` app. Anonymous sign-in still must be enabled in the staging Firebase Authentication console (the CLI has no provider-management command) before a live staging sign-in/admission test can run.
...
4. **Updated 2026-09-14:** Firebase CLI authentication is now complete and access to `powerglove-1cd23` plus its `digitable-staging-web` app is verified. Enable **Anonymous** under Firebase Console → Authentication → Sign-in method for that staging project before running a live sign-in test. The Firebase CLI's `auth` group only imports/exports accounts and cannot manage sign-in providers. Keep production uncreated.
