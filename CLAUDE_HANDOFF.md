...
- **Last updated:** 2026-09-25 by the hourly factory follow-up after rerunning the complete candidate gate and a live staging GM/player/table smoke
...
- **Fresh hourly verification (2026-09-25, follow-up rerun):** `digitable-staging-playthrough` is absent from the Claude roster and its supervisor is unreachable, so that missing process was not treated as completion. On the clean reviewed reskin candidate (`c220dd2`), `npm run check` passed (**692** tests, **11** todo; 71 files passed and 1 skipped), `npm run build` passed (existing Vite large-chunk warning only), and the complete emulator suite passed (**18** rules/testing, **86** Functions, **4** web tests). The isolated emulator emitted only known Admin-SDK metadata lookup warnings. A new read-only deployed-staging run, `node scripts/playtest/two-device-smoke.mjs --base https://digitable.signal-bleed.com --out /private/tmp/digitable-staging-playthrough-20260925-0222 --reload --port 9522`, then passed all **17/17** GM/player/table steps, with zero console errors, failed requests, or positive horizontal overflow at 375/768/1024/1280/1920 px; its report and 21 screenshots remain in that directory. This verifies the deployed pre-reskin staging build and does not deploy or merge the reskin candidate.
- **Next action:** John conducts the complete flow on two physical devices, including a lost-identity recovery attempt from a fresh/private browser, and records device/browser evidence. Do not deploy the reskin candidate, merge the stacked PRs, or promote to a separate production Firebase project without explicit direction.
