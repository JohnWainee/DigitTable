...
- **Last updated:** 2026-09-25 by Sonnet after a fifth independent audit of the mobile pop-out defect area found no new issue; no code, source, or deployment change occurred (see "Fifth independent review of the mobile pop-out defect" below).
...
- `node scripts/playtest/two-device-smoke.mjs --base https://digitable.signal-bleed.com --out /private/tmp/digitable-hourly-staging-20260922-final --reload` — all 17 live GM/player/table steps passed at 2026-09-22T17:33Z; report captures zero console/request errors and no horizontal overflow at 375/768/1024/1280/1920 px.
...
## Independent audit: reskin mobile pop-out defect (branch `sonnet-w/reskin-audit-20260925`, 2026-09-24)

**Scope:** re-audit the ink-black reskin and every GM/player/table flow for the reported mobile pop-out defect (select/menu/disclosure/modal/drawer/option-list/allocation/action controls under real phone viewports — 44px targets, no clipping/zoom/overflow), starting from a diff against the last fully independently-reviewed reskin evidence (`docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md`, `docs/evidence/sonnet-d-reskin/`, commit `930e3f1`) rather than re-litigating already-resolved findings.

**Method:** `git diff 930e3f1..HEAD` shows `apps/web/src/styles.css`, `apps/web/src/shared/**`, `apps/web/index.html`, and `scripts/playtest/ui-audit.mjs` are byte-for-byte unchanged since the reviewed reskin — so the pop-out sheet (`SheetDialog`), the six native `<select>`s, and the one `<details>` disclosure carry no new source changes and no new pop-out-shaped markup (`select`/`details`/`dialog`/`sheet`/`popover`/`menu`) was added anywhere in `apps/web/src` since that commit. Two feature commits landed afterward: `5e8907b` (sourcebook roster: `ChooseInjuryPanel2.tsx`, `ComposeStep2.tsx`, `PlayerDashboardScreen.tsx`) and `4083123` (recovery/credential-storage fix: `JoinScreen.tsx`, `session/**`). Neither is a pop-out control, but both add new interactive controls, so I checked them against the reskin's own tap-target contract instead of assuming coverage.

**Finding (real, in scope, now fixed):** `5e8907b` added two `<button type="button" onClick={...}>` elements with **no `className` at all** — "Destroy Cowboy hat to ignore this result" (`ChooseInjuryPanel2.tsx`) and "Mark and regain Blood" (`ComposeStep2.tsx`), the utility-item action controls in the injury-choice and compose-action flows. Every other button in the app is either `.primary-action`/`.secondary-action`/`.link-button` or one of the `.stepper-controls` +/- buttons (the one documented class-less exception, sized via a descendant selector) — all of which get `min-height`/`min-width: var(--tap)` (48px) and `touch-action: manipulation`. These two new buttons get neither: bare `button { font: inherit; }` is their only rule, so on a real phone they render at native intrinsic size, under the 44px WCAG 2.5.8 target and under the reskin's own 48px `--tap` budget — the exact class of defect this audit was asked to find. The reskin's own static contract test (`apps/web/test/styles/reskinContract.test.ts`, "covers every styled `<button>`...") has a blind spot: its regex only inspects buttons that already have a `className` attribute, so a wholly class-less button outside `.stepper-controls` passed silently.

**Fix (this branch, narrow):**
- `apps/web/src/player2/ChooseInjuryPanel2.tsx` and `apps/web/src/player2/ComposeStep2.tsx`: both buttons now carry `className="link-button"` — the existing underlined inline-action style, already tap-sized and already used nowhere else in the app (previously CSS-only, dead in markup).
- `apps/web/test/styles/reskinContract.test.ts`: added `stepperControlsRanges()` (balanced-tag ranges for every `.stepper-controls` block) and a new test, "never adds a `<button>` with no className at all outside `.stepper-controls`," that scans every `.tsx` file for class-less `<button>`s outside those ranges. Mutation-verified: reverting either `className="link-button"` addition makes this new test fail at the exact file/line; both were confirmed to fail before restoring the fix.
- No other file changed. `packages/*`, `templates/*`, `apps/functions`, rules, `styles.css`, `SheetDialog`, and every previously-reviewed pop-out control are untouched.

**Gates run on this branch (from `/private/tmp/digitable-sonnet-w-reskin-audit-20260925`, a fresh `npm install`):**
- `npm run check` — format, lint (zero warnings), typecheck, and **691 tests passed | 11 todo** (71 files passed, 1 skipped; baseline 690 + 1 new test).
- `npm run build` — clean (`apps/functions` esbuild 216.6kb; `apps/web` vite build; existing non-blocking >500kB chunk warning unchanged).
- `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — **18/18** `packages/testing`, **86/86** `apps/functions`, **4/4** `apps/web` (108/108), all against the local `demo-digitable` Auth/Firestore/RTDB/Functions emulators.
- `git diff --check` — clean. `package-lock.json` churn from the local `npm install` (a `"peer": true` metadata diff, unrelated to this change) was discarded, not committed.
- Not run: `scripts/playtest/ui-audit.mjs` (needs a real headless Chrome plus the Firebase emulators serving a built `apps/web`; not attempted in this pass — see "fresh live evidence" below).

**Independent review: satisfied.** A fresh independent Sonnet review of commit `5663d7f` (parent `b599abd`) approved the fix: no blocker in semantics, privacy, or authorization; independently reproduced `npm run check` (691 passed | 11 todo), `npm run build`, and the mutation check. One non-blocking future-robustness note (the new static test's class-less-button detection would not flag an explicitly *empty* `className`, though no current control has one). Full record: [`docs/reviews/2026-09-24-sonnet-w-utility-item-tap-target-review.md`](docs/reviews/2026-09-24-sonnet-w-utility-item-tap-target-review.md).

**Fresh browser evidence: satisfied locally (2026-09-25).** The first attempt exposed a test-harness regression instead of a UI defect: `ui-audit.mjs` hard-coded a visible `Rook` roster row, but the sourcebook roster now displays Iryna while retaining the internal `rook` ID. The harness now selects the first enabled exact-text `Correct` control in `.roster-panel-list`, and a regression pin prevents restoring the old `^rook` selector. The complete rerun used a fresh Chrome debug port and a production-built app served locally against the `demo-digitable` emulator stack: [`report.json`](/private/tmp/digitable-sonnet-w-post-utility-ui-audit-complete/report.json) records 150 states, 14 modal/sheet scenarios, 1,530 controls, zero control or overflow findings, zero hard axe violations, and zero console/request errors. It verifies the recovery form, both utility-item buttons, all role flows, phone/tablet/desktop/table widths, visual-viewport/safe-area/keyboard/zoom cases, and reduced motion. `npm run check` passed **692 active tests | 11 todo**; `npm run build` passed (existing chunk warning only); the emulator suite passed **18 + 86 + 4** tests. Fresh independent Sonnet review approved the harness change with no P0/P1; its sole P2 note is that the narrow regression pin is static rather than a DOM-selector unit test, which is mitigated by this real-browser run. See [`docs/reviews/2026-09-25-sonnet-w-ui-audit-roster-selector-review.md`](docs/reviews/2026-09-25-sonnet-w-ui-audit-roster-selector-review.md). No merge or deployment occurred.

**No other in-scope defect found.** The sheet/select/disclosure pop-out system itself is unchanged and remains exactly as reviewed in `docs/reviews/2026-09-18-sonnet-d-reskin-independent-review.md`; its two accepted, already-documented non-blocking limits (320px + 200% text-only geometry; iOS Safari visual-viewport-keyboard and physical-device evidence still open) are unchanged and are not new defects. Left the worktree otherwise clean — no handoff-only churn beyond recording this finding.

## Third independent review of the tap-target/audit-selector candidate (branch `sonnet-y/reskin-integration-20260925`, 2026-09-25)

**Scope:** a fresh, isolated Sonnet reviewer (no access to prior sessions' reasoning) was asked to determine whether commits `5663d7f`/`5cdee1a`/`c77cd94` (the utility-item tap-target fix, its independent review, and the roster-agnostic audit selector) are a coherent, non-duplicative improvement over `factory/today-integration`, and to independently audit interactive controls (menus, disclosures, modals, drawers, pop-outs, allocation/action pickers) at phone (320/375px, dynamic viewport/keyboard), tablet, desktop, and table widths.

**Finding: confirmed genuine, non-duplicative delta.** `git log factory/today-integration..HEAD` shows exactly these three commits are absent from `factory/today-integration` (`841099a`); the two commits `factory/today-integration` has that this branch lacks (`841099a`, `a350b2d`) are unrelated docs-only staging-smoke records. Directly reading `apps/web/src/player2/ChooseInjuryPanel2.tsx` and `ComposeStep2.tsx` as they exist on `factory/today-integration` confirms both class-less `<button>` elements are still unpatched there today — the defect this candidate fixes is live on the deployed/staging comparison branch, not already resolved by a different path.

**Independent re-verification of the fix and its evidence (not just its prose):** confirmed `.link-button` genuinely receives `min-height`/`min-width: var(--tap)` (48px, scales with root font) plus `touch-action: manipulation` in `apps/web/src/styles.css`; read the new `reskinContract.test.ts` class-less-button test in full and confirmed its `stepperControlsRanges()` depth-counting is correct against both real `.stepper-controls` consumers; independently inspected (not just cited) `/private/tmp/digitable-sonnet-w-post-utility-ui-audit-complete/report.json` and confirmed `ok: true`, `states: 150`, `controlsAudited: 1530`, `controlIssues: 0`, `overflowStates: 0`, `axeHardViolations: 0`, `failures: 0`, covering exactly the required phone-small(320)/phone(375)/phone-landscape/tablet(768)/desktop(1280)/table(1920) matrix plus dynamic-viewport-keyboard, safe-area, and 150%/200% text-scaling scenarios (all read directly from `scripts/playtest/ui-audit.mjs`'s `VIEWPORTS` and scenario bodies). Confirmed no pop-out control (`SheetDialog`, native `<select>`s, the one `<details>`) has any source change since `930e3f1`, from this branch's own tip.

**Gates reproduced this session:** `npm run check` — **692 tests passed | 11 todo** (71 files, 1 skipped), matching the branch's recorded result. `npm run build` — clean, same non-blocking chunk warning. `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — the port conflict noted in this review's first pass cleared once the concurrent sandbox process released them; rerun cleanly and **passed 108/108** (18 `packages/testing`, 86 `apps/functions`, 4 `apps/web`).

**Fresh live evidence, generated by this session (not just cited):** a from-scratch live-mode build (`VITE_FIREBASE_USE_EMULATOR=true` against a fake `demo-digitable` config, `127.0.0.1`-only, no LAN exposure) served via `vite preview` on `127.0.0.1:4174` against the local Auth/Firestore/Functions emulators, then `node scripts/playtest/ui-audit.mjs --base http://127.0.0.1:4174 --label sonnet-y-independent --out <job-tmp>/ui-audit-out --port 9761 --no-shots`: `ok: true`, **150 states**, **1,470 controls audited**, **0 control issues**, **0 overflow states**, **0 hard axe violations**, **0 failures**, **14/14 modal/sheet scenarios passed** (phone-small/phone/phone-landscape/tablet/desktop/table widths, dynamic-viewport-keyboard emulation, pinch-zoom, safe-area insets, 200% text, internal scroll-to-control, reduced-motion), zero console errors/failed requests across gm/player/table/anon. Only the same two pre-existing, already-documented, non-gating items recorded (320px+200%-text geometry note; the intentional nonexistent-room route's heading best-practice note). Background emulator/preview processes were stopped afterward; nothing was committed from the emulator-mode build.

**Disposition: approve, no blocker, no further source change.** Full record: [`docs/reviews/2026-09-25-sonnet-y-reskin-integration-independent-review.md`](docs/reviews/2026-09-25-sonnet-y-reskin-integration-independent-review.md). This is the third independent pass over this exact candidate; all three (`docs/reviews/2026-09-24-sonnet-w-utility-item-tap-target-review.md`, `docs/reviews/2026-09-25-sonnet-w-ui-audit-roster-selector-review.md`, and this one) agree with no blocking finding, and this pass is now the first of the three to carry fresh, from-this-session evidence for every required gate (format/lint/typecheck/unit, build, full emulator suite, real-browser accessibility/geometry audit). The candidate is ready to fold into `factory/today-integration` at John's discretion; this review does not merge, deploy, or promote anything, and no code outside this handoff and the review file was changed.

## Fourth independent review of the mobile pop-out defect (branch `sonnet-ad/reskin-hourly-20260925`, 2026-09-25)

**Scope:** a fresh, isolated Sonnet reviewer, starting from `factory/today-integration` at the now-merged `d5f9bce` (which folds in the tap-target fix and the roster-agnostic audit selector three prior independent passes already approved), was asked to independently re-audit every select, menu, disclosure, modal, drawer, option list, and action/allocation picker at real phone/landscape/tablet/desktop/table viewports, including dynamic-viewport-keyboard, safe-area, zoom/text-scaling, touch, and keyboard behavior.

**Finding: no new defect; no code change made.** `apps/web/src/styles.css`, `apps/web/src/shared/**`, and `apps/web/index.html` remain byte-for-byte unchanged since `930e3f1` — the pop-out system itself (`SheetDialog`, the six native `<select>`s, the one `<details>` disclosure) carries no source change since the last full reskin review. Both previously-fixed class-less buttons (`ChooseInjuryPanel2.tsx`, `ComposeStep2.tsx`) confirmed still carrying `className="link-button"` at `d5f9bce`, independently re-grepped, not cited. `PlayerDashboardScreen.tsx`'s post-`930e3f1` diff (utility-item wiring) adds no new interactive element of its own.

**Gates reproduced this session** (fresh `npm install`, fresh worktree): `npm run check` — **692 tests passed | 11 todo** (71 files, 1 skipped). `npm run build` — clean. `PATH=/opt/homebrew/opt/openjdk/bin:$PATH npm run test:emulator` — **108/108** (18 + 86 + 4). `git diff --check` — clean (the same immaterial `package-lock.json` `"peer": true` churn from a fresh install was reverted, not committed).

**Fresh live evidence generated by this session:** a from-scratch emulator-mode build (`VITE_FIREBASE_USE_EMULATOR=true`, fake `demo-digitable` config, `127.0.0.1`-only — no LAN exposure) served via `vite preview`, audited with `node scripts/playtest/ui-audit.mjs --base http://127.0.0.1:4174 --label sonnet-ad-independent --port 9762 --no-shots` against the local Auth/Firestore/Functions emulators: `ok: true`, **150 states**, **1,434 controls audited** (covers every native `<select>`, the `<details>` disclosure, the `role="spinbutton"` allocation stepper, every button/link/input), **0 control issues**, **0 overflow states**, **0 hard axe violations**, **0 failures**, **14/14 modal/sheet scenarios** (phone-small(320)/phone(375)/phone-landscape/phone-landscape(667×375)/tablet(768)/desktop(1280) plus dynamic-viewport-keyboard, pinch-zoom, safe-area, 200% text, reduced-motion) each independently checked against all eight geometry/behavior invariants (`dialogInsideViewport`, `actionsVisibleWithoutScrolling`, `noPageOverflow`, `actionTargets44`, `reasonReachableAfterScroll`, `rootScrollLocked`, `backgroundInert`, `focusInsideDialog`), all `true` except the one already-accepted 320px+200%-text extreme (the audit script itself logs this as `INFO ... recorded, not gating`, matching every prior review's documented, non-blocking limit — not a new finding). Zero console errors or failed requests across gm/player/table/anon. Reduced-motion capture confirms the sheet animation collapses under `prefers-reduced-motion: reduce`. Background emulator/preview processes stopped afterward; nothing committed from the emulator-mode build.

**Disposition: no defect found, no code change, nothing to merge or deploy.** This is the fourth independent pass over this exact defect area and the first to run against the now-merged `d5f9bce` base; it agrees with all three prior passes (`docs/reviews/2026-09-24-sonnet-w-utility-item-tap-target-review.md`, `docs/reviews/2026-09-25-sonnet-w-ui-audit-roster-selector-review.md`, `docs/reviews/2026-09-25-sonnet-y-reskin-integration-independent-review.md`). Full record: [`docs/reviews/2026-09-25-sonnet-ad-mobile-popout-independent-review.md`](docs/reviews/2026-09-25-sonnet-ad-mobile-popout-independent-review.md). Physical-device evidence (real iOS/Android hardware, real Safari visual-viewport keyboard behavior) remains the one open item for John, unchanged by this pass — see `docs/PLAYTEST_TWO_DEVICE.md`.

## Fifth independent review of the mobile pop-out defect (branch `sonnet-af/reskin-fresh-20260925`, 2026-09-25)

**Scope:** a fresh, isolated Sonnet reviewer, starting from `factory/today-integration` at `2452f66` (docs-only, records the fourth review), independently re-audited every mobile select, menu, disclosure, modal, drawer, option list, and action/allocation picker — dynamic-viewport-keyboard, safe areas, 44px touch targets, scroll/focus behavior, reduced motion, horizontal overflow, and GM/player/table state semantics — with special attention to genuinely re-deriving evidence rather than citing the four prior passes.

**Finding: no new defect; no code change made.** `git diff 930e3f1..HEAD` still shows `apps/web/src/styles.css`, `apps/web/src/shared/**`, and `apps/web/index.html` byte-for-byte unchanged (only `scripts/playtest/ui-audit.mjs`'s roster-agnostic selector differs) — the pop-out system (`SheetDialog`, the six native `<select>`s across `SceneDirector.tsx`/`GmToolsPanel.tsx`, the one `<details>` disclosure in `ComposeStep2.tsx`, the `AllocationStepper` spinbutton) carries no source change since the last fully independently reviewed reskin evidence. `SheetDialog.tsx` and `AllocationStepper.tsx` were read in full this session (portal mount, inert-sibling trap, visual-viewport sizing, root scroll lock, focus management, Tab-cycling restricted to reachable descendants, keyboard-reveal `scrollIntoView`; tap +/- buttons plus full Arrow/Home/End keyboard support). `GmToolsPanel`/`SceneDirector` (all six selects, every GM-only command form) confirmed reachable only from `GmDirectorScreen`, not from any player/table surface.

**Gates reproduced this session** (fresh worktree, fresh `npm install`): `npm run check` — **692 tests passed | 11 todo** (71 files, 1 skipped). `npm run build` — clean. `git diff --check` — clean (the same immaterial `package-lock.json` `"peer": true` churn from a fresh install was reverted, not committed). `npm run test:emulator` not rerun standalone (no `packages/testing`/`apps/functions`/rules/engine file changed); superseded by the live audit below.

**Fresh live evidence generated by this session, including one recorded environment hiccup:** the first live-audit attempt (`VITE_FIREBASE_USE_EMULATOR=true` build, local Auth/Firestore/Functions emulators, `vite preview` on `127.0.0.1:4174`, `node scripts/playtest/ui-audit.mjs`) failed at "timed out waiting for: player reveal" after 60/150 states — diagnosed as sandbox port contention from a concurrent, unrelated background session's own `firebase emulators:exec` run on the same default ports (this repo's `firebase.json` fixes 8080/9099/9000/5001), which sent this session's Firestore emulator a `SIGTERM` mid-run — the same class of contention `docs/reviews/2026-09-25-sonnet-y-reskin-integration-independent-review.md` already recorded, not an application defect. After restarting the local emulator stack and `vite preview` and confirming both healthy, the identical rerun passed cleanly: `ok: true`, **150 states**, **1,422 controls audited**, **0 control issues**, **0 overflow states**, **0 hard axe violations**, **0 failures**, **14/14 modal/sheet scenarios** (phone-small(320)/phone(375)/phone-landscape(812×375, 667×375)/tablet(768)/desktop(1280) plus dynamic-viewport-keyboard, pinch-zoom, safe-area insets, 150%/200% text, reduced-motion), every one of the eight geometry/behavior invariants `true` per scenario except the one already-accepted 320px+200%-text extreme (`INFO ... recorded, not gating`, matching every prior review). Zero console errors or failed requests across gm/player/table/anon. `reducedMotion` capture confirms the sheet animation collapses under `prefers-reduced-motion: reduce` (`0.18s` → `1e-06s`). Background emulator/preview processes stopped afterward; nothing committed from the emulator-mode build.

**Disposition: no defect found, no code change, nothing to merge or deploy.** This is the fifth independent pass over this exact defect area; it agrees with all four prior passes (`docs/reviews/2026-09-24-sonnet-w-utility-item-tap-target-review.md`, `docs/reviews/2026-09-25-sonnet-w-ui-audit-roster-selector-review.md`, `docs/reviews/2026-09-25-sonnet-y-reskin-integration-independent-review.md`, `docs/reviews/2026-09-25-sonnet-ad-mobile-popout-independent-review.md`). Full record: [`docs/reviews/2026-09-25-sonnet-af-mobile-popout-independent-review.md`](docs/reviews/2026-09-25-sonnet-af-mobile-popout-independent-review.md). Physical-device evidence (real iOS/Android hardware, real Safari visual-viewport keyboard behavior) remains the one open item for John, unchanged by this pass.
